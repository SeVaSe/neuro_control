...
class LowerOrthosesPage extends StatefulWidget {
  const LowerOrthosesPage({Key? key}) : super(key: key);

  @override
  State<LowerOrthosesPage> createState() => _LowerOrthosesPageState();
}

class _LowerOrthosesPageState extends State<LowerOrthosesPage> {
  ...
  Future<void> loadOrthoses() async {
    try {
      ...
      final String response = await rootBundle
          .loadString('lib/assets/data/json/lower_limb_orthoses.json');
      ...
      if (data != null && data['orthoses'] != null) {
        ...
        print('Количество загруженных ортезов: ${orthoses.length}');
      } else {
  ...
  List<Orthosis> get filteredOrthoses {
    ...
    if (timeFilter != 'Все') {
      filtered = filtered
          .where((orthosis) => orthosis.timeType == timeFilter)
          .toList();
    }
              ...
              children: [
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: Row(
                      children: ['Все', 'Дневной', 'Ночной', 'Дневной/Ночной']
                          .map((filter) {
            ...
            child: isLoading
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 8),
                  Text(
                    'Проверьте файл lower_limb_orthoses.json',
                    style: TextStyle(
                      fontSize: 14,
                      color: Colors.grey,
                    ),
              ...
              child: GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return OrthosisCard(
                    orthosis: orthosis,
                    onTap: () =>
                        _showOrthosisDetails(context, orthosis),
                  );
                },
              ),
            ),
...
class OrthosisDetailModal extends StatelessWidget {
  ...
  const OrthosisDetailModal({Key? key, required this.orthosis})
      : super(key: key);
